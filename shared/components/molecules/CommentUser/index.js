...
class CommentUser extends React.Component {
  ...
  showReplyBox = () => {
    replyBox = (
      <div style={{ ...styles.cont, margin: '15px 0 15px 60px' }}>
        <ReplyBox />
        <RaisedButton
          onClick={this.reply}
          label="Reply"
        ...
        <div>
          <Link to="#">
            <Avatar
              src="/tmp/avatar.png"
              size={49}
              style={{ float: 'left', marginRight: '15px', display: 'block' }}
            />
          </Link>
        </div>
        <DetBox>
          <Name className="sans-font">
            {this.props.data.name} <Time>{this.props.data.date}</Time>
          </Name>
          <div className="sans-font text-detail">
            <Tag>@{this.props.data.target}</Tag> {this.props.data.text}
          </div>
          <BtnBox>
            <RaisedButton
              label="Upvote | 18"
          ...
          <TextBtn2>
            <FontIcon
              ...
              style={{ color: '#C1C1C1', fontSize: '14px', marginRight: '10px' }}
            >
              favorite_border
            </FontIcon>
            {' '}
