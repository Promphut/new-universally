...
class NewStory extends React.Component {
  ...
  autoSave = () => {
    ...
    if (status === this.SAVE_STATUS.DIRTIED) {
      ...
      if (story.format === 'NEWS') {
        ...
        if (story.column === newsCol || (story.column && story.column._id === newsCol)) { story.column = prevStory.column ? prevStory.column : null; }
  ...
  render() {
    ...
    const saving = status === this.SAVE_STATUS.DIRTIED;
    const topRight = (
      <EditorTopRight
        story={story}
        prevStory={prevStory}
