...
app.get(['/feed', '/feed/rss', '/rss'], (req, res) => {
  ...
  api
    .getFeed(type.toLowerCase(), { status: 1 }, 'latest', null, null, 20, { rss: true })
    .then((result) => {
...
app.get(['/feed/atom', '/atom'], (req, res) => {
  let type = req.query.type || 'article';
  api
    .getFeed(type.toLowerCase(), { status: 1 }, 'latest', null, null, 20, { atom: true })
    .then((result) => {
      res.set('Content-Type', 'text/xml');
      res.send(result.xml);
    });
...
app.get(['/linetoday', '/feed/linetoday'], (req, res) => {
  let type = req.query.type || 'article';
  api
    .getFeed(type.toLowerCase(), { status: 1 }, 'latest', null, null, 20, { line: true })
    .then((result) => {
      res.set('Content-Type', 'text/xml');
      res.send(result.xml);
    });
