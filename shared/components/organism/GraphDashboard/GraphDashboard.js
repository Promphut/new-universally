...
class LineChart extends React.Component {
  ...
  render() {
    ...
    let x = d3.scaleTime().domain(d3.extent(data, d => d.date)).range([30, w]),
      y = d3.scaleLinear().domain([0, d3.max(data, d => d.value)]).range([h, 0]);
    ...
    let xAxis = d3.axisBottom(x).tickSize(-h - 20).ticks(8),
      yAxis = d3
        ...
        .tickSize(-w - 20)
        .ticks(6)
        .tickFormat(tickVal => (tickVal >= 1000 ? `${tickVal / 1000}K` : tickVal));
    ...
    let area = d3.area().curve(d3.curveLinear).x(d => x(d.date)).y0(h).y1(d => y(d.value));

    let line = d3.line().curve(d3.curveLinear).x(d => x(d.date)).y(d => y(d.value));
