...
class EditorContent extends React.Component {
  ...
  render() {
    ...
    column = column === menu.columns.find(col => col.name === 'news')._id ? null : column;
    type = type === 0 ? null : type;

    return (
      <Container>
        <RadioContainer>
          <RadioButtonGroup name="format" defaultSelected={format} onChange={changeFormat}>
            <RadioButton
              value="NEWS"
              label="News"
              ...
              >
                {columns.length != 0 &&
                    columns.map(
                      (data, index) =>
                        (data.name !== 'news'
                          ? <MenuItem key={index} value={data._id} primaryText={data.name} />
                          : null),
                    )}
              </SelectField>
            </ColumnContainer>
