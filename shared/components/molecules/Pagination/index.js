...
const Page = ({ value, isActive, onClick }, context) => {
  ...
  return (
    <FlatButton
      style={
        !isActive
          ? { ...flatButtonStyle, padding: '0px 0px 35px 0px' }
          : { ...flatButtonStyle, ...active, color: acc, padding: '0px 0px 35px 0px' }
      }
      labelStyle={
        !isActive
          ? { fontSize: utils.isMobile() ? '14px' : '16px', padding: '0px 10px 0px 10px' }
          : {
...
const Ellipsis = ({ onClick }) => (
  <FlatButton
    style={{ ...flatButtonStyle, padding: '0px 0px 35px 0px' }}
...
const FirstPageLink = ({ isActive, onClick }) => (
  <FlatButton
    ...
    icon={
      <NavigationFirstPage
        style={{
          width: utils.isMobile() ? '20px' : '24px',
          height: utils.isMobile() ? '20px' : '24px',
        }}
      />
    }
    onClick={onClick}
