...
const ExtractMeta = (setting, url) => {
  ...
  if (setting.publisher.name) {
    meta.name =
      setting.publisher.name + (setting.publisher.tagline ? ` | ${setting.publisher.tagline}` : '');
  }
  ...
  if (
    ...
    let user;
    if (path[1].indexOf('?') != -1) { user = decodeURIComponent(path[1].substring(1, path[1].indexOf('?'))); } else user = decodeURIComponent(path[1].substring(1));
    return api
    ...
    let uid;
    if (path[2].indexOf('?') != -1) uid = path[2].substring(path[2].indexOf('?'));
    else uid = path[2];
    ...
    // 5. Tags case
    let tag;
    if (path[2].indexOf('?') != -1) tag = path[2].substring(0, path[2].indexOf('?'));
    else tag = path[2];
    return api.getTagFromTagSlug(decodeURIComponent(tag)).then().catch(err => ({ status: 404 }));
  } return Promise.resolve(meta);
};
