    ...
    <Container style={{ ...style }}>
      ...
      <BoxText className="sans-font">
          ...
          {column &&
            <Div>
              A story of <span style={{ textDecoration: 'underline' }}>
                <Link to={column.url}>{column.name}</Link>
              </span>
            </Div>}
        </DivDes>
        <BGImg
        ...
        <Div style={{ margin: '10px 0 0 0' }}>
          ...
          Votes
          {' '}
          <span style={{ marginLeft: '15px' }}>{comments.count} Comments</span>
          {' '}
          {readTime && <span style={{ float: 'right' }}>Read {readTime} min</span>}
        </Div>
      </BoxText>
    </Container>
  );
