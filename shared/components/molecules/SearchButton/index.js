...
const getStyles = (name, theme, scrolling, transparent, hasCover) => {
  if (name == 'hintStyle') {
    return {
      color: (!scrolling && transparent && hasCover) || theme == 'dark'
...
class SearchButton extends React.Component {
  ...
  handleChange = (event) => {
    ...
    if (value != this.state.text) this.setState({ text: value });
  };
  ...
  componentWillReceiveProps(nextProps) {
    if (this.state.redirect) this.setState({ redirect: false, text: '' });
  }
  ...
  render() {
    ...
    return (
      <div>
        ...
        <MuiThemeProvider
          muiTheme={
            (!this.props.scrolling && this.props.transparent && this.props.hasCover) ||
              theme.barTone == 'dark'
              ? muiTheme2
              : muiTheme
          }
        >
          <ButtonContainer mar={utils.isMobile() ? '8px' : '32px'}>
            {utils.isMobile()
              ? <Link to="/search/stories?keyword=">
                <SearchButtonIcon id="bt" className="fa fa-search" aria-hidden="true" />
              </Link>
              : <Button>
                <SearchButtonIcon
            ...
            {this.state.focus &&
              <TextField
                ...
                floatingLabelFocusStyle={getStyles(
                  'inputStyle',
                  theme.barTone,
                  theme.barTone,
                  this.props.scrolling,
