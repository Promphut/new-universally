...
class UploadPicture extends React.Component {
  ...
  isFilesizeValid = file => file.size <= this.maxMB * 1000000;
  ...
  upload = (e) => {
    ...
    reader.onloadend = (event) => {
      if (this.props.crop) {
        ...
        api
          .uploadFile(file, this.props.type, config.BACKURL + this.props.path, null)
          .then((res) => {
            this.setState({
  ...
  uploadToServer = () => {
    ...
    api
      .uploadFile(this.file, this.props.type, config.BACKURL + this.props.path, this.state.data)
      .then((res) => {
        this.setState({
          msg: 'Upload',
          err: false,
  ...
  render() {
    ...
    return (
      <Container style={{ ...style, width, height: `${height + 20}px` }}>
        <ConfirmDialog
        ...
        <Des className="sans-font" style={{ color: err ? '#D8000C' : '#c2c2c2' }}>
          {msg}
        </Des>
        <input
          type="file"
