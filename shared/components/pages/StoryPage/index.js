...
class StoryPage extends React.Component {
  ...
  render() {
    ...
    let { recommends, description, showTopbarTitle, canEditStory } = this.state;
    ...
    if (!isEmpty(story)) {
      if (isMobile) {
        likeBoxSize = 300;
        nextStoryContainer = <NextStoryMobile currentID={story._id} />;
      } else {
        nextStoryContainer = <NextStory currentID={story._id} />;
      }
    }
    var story = {};
    if (this.props.story && this.props.story._id == this.props.match.params.sid) {
          ...
          <Content paddingTop={hasCover ? '0px' : '60px'}>
            ...
            <Main ref={'TT'} isMobile={isMobile}>
              ...
              {isMobile &&
                ...
                />}
              {recommends.length > 1 && <RecommendContainer stories={recommends} />}
            </Main>
