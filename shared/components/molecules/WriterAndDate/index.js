      ...
      <Left>
        {!isMo &&
          <div>
            <a href={writer.url || '#'}>
              <Avatar
                src={writer.pic && writer.pic.medium}
                size={54}
                className="imgWidth"
                style={{ float: 'left' }}
              />
        ...
        <Contain className="sans-font">
          by
          {' '}
          <Span1><a href={writer.url || '#'} style={{ color: '#222' }}>{writer.display}</a></Span1>
          ,
          {isMo
            ? <span><br />{moment(published).format('lll')}</span>
            : <span>{moment(published).format('lll')}</span>}
          <br />
