...
// keyword is optional
api.getUsers = keyword => Request.get(`${config.BACKURL}/users`)
    .query({
      keyword,
    })
    .set('Accept', 'application/json')
...
api.getPublisherColumns = () => Request.get(`${config.BACKURL}/publishers/${config.PID}/columns`)
    .set('Accept', 'application/json')
...
api.getUserFromUsername = username => Request.get(`${config.BACKURL}/slugs/users/${encodeURIComponent(username)}`)
    .set('Accept', 'application/json')
    .then(res => res.body.user)
    .catch(err => api.userNotFoundPromise());

api.getUserFromUserId = uid => Request.get(`${config.BACKURL}/users/${uid}`)
    .set('Accept', 'application/json')
    .then(res => res.body.user)
    .catch(err => api.userNotFoundPromise());
...
// token is optional
api.getColumn = (cid, token) => Request.get(`${config.BACKURL}/publishers/${config.PID}/columns/${cid}`)
    .set('Accept', 'application/json')
    .set('x-access-token', token || '')
    .then(res => res.body.column);

api.updateColumn = (cid, column) => Request.patch(`${config.BACKURL}/publishers/${config.PID}/columns/${cid}`)
...
api.getStoryFromKeyword = (keyword, type, page, limit) => Request.post(`${config.BACKURL}/stories/${config.PID}/find`)
    .query({
      page,
      limit,
    })
...
api.getFocusWordDetail = focusWord => Request.get(`${config.BACKURL}/stories/getfocusword/${focusWord}`)
    .set('Accept', 'application/json')
    .then(res => res.body, new Error('Get Focus Word Detail Error'));

api.getCookieAndToken = token => Request.get(`${config.BACKURL}/publishers/${config.PID}/menu`)
    .query({
      token,
    })
    .set('Accept', 'application/json')
...
api.signin = data => Request.post(`${config.BACKURL}/auth`)
    .send(data)
    .set('Accept', 'application/json')
    .then(res => res.body, api.err);

api.signup = data => Request.post(`${config.BACKURL}/users`)
    .send(data)
    .set('Accept', 'application/json')
    .then(res => res.body, api.err);
...
api.getColumns = (status = 1) => Request.get(`${config.BACKURL}/publishers/${config.PID}/columns`)
    .query({
      status,
    })
    .set('Accept', 'application/json')
...
api.removeColumn = cid => Request.delete(`${config.BACKURL}/publishers/${config.PID}/columns/${cid}`)
    .set('x-access-token', auth.getToken() || '')
    .set('Accept', 'application/json')
    .then(res => res.body);

api.newColumn = col => Request.post(`${config.BACKURL}/publishers/${config.PID}/columns`)
    .set('x-access-token', auth.getToken() || '')
    .set('Accept', 'application/json')
    .send({
      column: col,
...
api.getTagFromTagId = tid => Request.get(`${config.BACKURL}/publishers/${config.PID}/tags/${tid}`).then(res => res.body.tag, api.err);

api.getTagFromTagSlug = slug => Request.get(
    `${config.BACKURL}/slugs/publishers/${config.PID}/tags/${encodeURIComponent(slug)}`,
  ).then(res => res.body.tag, api.err);

api.getStoryTags = sid => Request.get(`${config.BACKURL}/stories/${sid}/tags`).then(res => res.body.tags, api.err);

api.getTags = () => Request.get(`${config.BACKURL}/publishers/${config.PID}/tags`).then(res => res.body.tags, api.err);

api.getTagFromSlug = slug => Request.get(
    `${config.BACKURL}/slugs/publishers/${config.PID}/tags/${encodeURIComponent(slug)}`,
  )
    .set('Accept', 'application/json')
...
api.addTag = name => Request.post(`${config.BACKURL}/publishers/${config.PID}/tags`)
    .set('x-access-token', auth.getToken() || '')
    .send({
      tag: {
        name,
...
api.removeTag = tid => Request.delete(`${config.BACKURL}/publishers/${config.PID}/tags/${tid}`)
    .set('x-access-token', auth.getToken() || '')
    .then(res => res.body.tag, api.err);

api.getPublisherWriters = () => Request.get(`${config.BACKURL}/publishers/${config.PID}/writers`)
    .set('Accept', 'application/json')
    .then(res => res.body.writers, api.err);

api.getPublisherNormalUsers = uid => Request.get(`${config.BACKURL}/publishers/${config.PID}/isNormalUser/${uid}`)
    .set('Accept', 'application/json')
    .then(res => res.body, api.err);
...
api.updateStory = (sid, story) => Request.patch(`${config.BACKURL}/stories/${sid}`)
    .set('x-access-token', auth.getToken() || '')
    .send({
      story,
    })
    .then(res => res.body.story, api.err);
api.createStory = story => Request.post(`${config.BACKURL}/stories`)
    .set('x-access-token', auth.getToken() || '')
    .send({
      story,
    })
...
api.getOtherPublisher = id => Request.get(`${config.BACKURL}/publishers/${id}`)
    .set('Accept', 'application/json')
    .then(res => res.body.publisher, api.err);
// token can be null in that case the public publisher object will be fetched.
api.getPublisher = token => Request.get(`${config.BACKURL}/publishers/${config.PID}`)
    .query({
      token,
    })
    .set('Accept', 'application/json')
    .then(res => res.body.publisher, api.err);
api.getPublisherContactCats = () => Request.get(`${config.BACKURL}/publishers/${config.PID}/contactcats`)
    .set('Accept', 'application/json')
    .then(res => res.body.publisher.contactCats || [], api.err);
api.getPublisherAboutUs = () => Request.get(`${config.BACKURL}/publishers/${config.PID}/aboutus`)
    .set('Accept', 'application/json')
    .then(res => res.body.publisher.aboutUs || '', api.err);
api.getPublisherSetting = () => Request.get(`${config.BACKURL}/publishers/${config.PID}/setting`)
    .set('Accept', 'application/json')
    .then(res => res.body, api.err);

api.updatePublisher = publisher => Request.patch(`${config.BACKURL}/publishers/${config.PID}`)
    .set('x-access-token', auth.getToken() || '')
    .set('Accept', 'application/json')
    .send({
      publisher,
...
api.newContactCat = contactCat => Request.post(`${config.BACKURL}/publishers/${config.PID}/contactcats`)
    .set('x-access-token', auth.getToken() || '')
    .set('Accept', 'application/json')
    .send({
      contactCat,
...
api.updateContactCat = (contactCat) => {
  ...
  return Request.patch(
    `${config.BACKURL}/publishers/${config.PID}/contactcats/${contactCat._id}`,
  )
    .set('x-access-token', auth.getToken() || '')
    .set('Accept', 'application/json')
    .send({
...
api.addEditorToColumn = (uid, cid) => {
  ...
  return Request.post(
    `${config.BACKURL}/publishers/${config.PID}/columns/${cid}/editors/${uid}`,
  )
    .set('x-access-token', token || '')
    .set('Accept', 'application/json')
    .then(res => res.body);
...
api.removeEditor = (editorId, cid) => Request.delete(
    `${config.BACKURL}/publishers/${config.PID}/columns/${cid}/editors/${editorId}`,
  )
    .set('x-access-token', auth.getToken() || '')
    .set('Accept', 'application/json')
    .then(res => res.body);

api.getEditors = cid => Request.get(`${config.BACKURL}/publishers/${config.PID}/columns/${cid}/editors`)
    .set('Accept', 'application/json')
...
api.addWriterToColumn = (uid, cid) => {
  ...
  return Request.post(
    `${config.BACKURL}/publishers/${config.PID}/columns/${cid}/writers/${uid}`,
  )
    .set('x-access-token', token || '')
    .set('Accept', 'application/json')
    .then(res => res.body);
...
api.removeWriter = (writerId, cid) => Request.delete(
    `${config.BACKURL}/publishers/${config.PID}/columns/${cid}/writers/${writerId}`,
  )
    .set('x-access-token', auth.getToken() || '')
    .set('Accept', 'application/json')
    .then(res => res.body);

api.getColumnWriters = cid => Request.get(`${config.BACKURL}/publishers/${config.PID}/columns/${cid}/writers`)
    .set('Accept', 'application/json')
    .then(res => res.body.writers);

api.sendContactEmail = (contactCat, message) => Request.post(`${config.BACKURL}/publishers/${config.PID}/contacts`)
    .set('Accept', 'application/json')
    .send({
      contactCat,
      contact: message,
...
api.updateFBshare = (sid, share_fb) => Request.post(`${config.BACKURL}/stories/${sid}/shares`)
    .set('Accept', 'application/json')
    .send({
      fb: share_fb,
    })
...
api.getContentTypes = () => Request.get(`${config.BACKURL}/publishers/${config.PID}/columns`)
    .set('Accept', 'application/json')
...
api.getViewInsight = (insight, subaction = '', filter, sort, limit, current) => {
  ...
  return Request.get(
    `${config.BACKURL}/insights/${insight}/${config.PID}/view/${subaction}`,
  )
    .set('x-access-token', token || '')
    .query({
      filter: filter ? JSON.stringify(filter) : null,
...
api.getTrendInsight = (insight, subaction = '', filter, sort, limit, current) => {
  ...
  return Request.get(
    `${config.BACKURL}/insights/${insight}/${config.PID}/trend/${subaction}`,
  )
    .set('x-access-token', token || '')
    .query({
      filter: filter ? JSON.stringify(filter) : null,
...
api.getShareInsight = (insight, subaction = '', filter, sort, limit, current) => {
  ...
  return Request.get(
    `${config.BACKURL}/insights/${insight}/${config.PID}/share/${subaction}`,
  )
    .set('x-access-token', token || '')
    .query({
      filter: filter ? JSON.stringify(filter) : null,
...
api.getGrowthInsight = (insight, subaction = '', filter, sort, limit, current) => {
  ...
  return Request.get(
    `${config.BACKURL}/insights/${insight}/${config.PID}/growth/${subaction}`,
  )
    .set('x-access-token', token || '')
    .query({
      filter: filter ? JSON.stringify(filter) : null,
...
api.shorten = (url, utm) => {
  ...
  return Request.post(
    `https://www.googleapis.com/urlshortener/v1/url?key=${config.ANALYTIC.GOOGLE_URL_SHORTENER}`,
  )
...
// hash is without #
api.createHash = (hash, sid) => Request.post(`${config.BACKURL}/publishers/${config.PID}/hashes`)
    .send({
      hash,
      story: sid,
    })
...
api.checkSignUp = email => Request.post(`${config.BACKURL}/signup/check`)
    .send({
      email,
    })
    .set('Accept', 'application/json')
...
api.filterStoryByTitle = (title, sort) => Request.post(`${config.BACKURL}/stories/find`)
    .send({
      title,
      sort,
    })
...
api.getChildren = () => Request.get(`${config.BACKURL}/publishers/${config.PID}/columns/parent`)
    .set('Accept', 'application/json')
    .then(res => res.body.columns, api.err);

api.getChildrenFromParent = parent => Request.get(
    `${config.BACKURL}/publishers/${config.PID}/columns/${parent}/parent`,
  )
    .set('Accept', 'application/json')
    .then(res => res.body.columns, api.err);

api.getSignature = sid => Request.get(`/get_signature/${sid}`).set('Accept', 'application/json').then(res => res.body, api.err);
api.getNextStory = sid => Request.get(`${config.BACKURL}/stories/${sid}/next`)
    .set('Accept', 'application/json')
    .then(res => res.body, api.err);
api.getRecommendStories = sid => Request.get(`${config.BACKURL}/stories/${sid}/recommend`)
    .set('Accept', 'application/json')
    .then(res => res.body, api.err);
api.deleteImage = url => Request.delete(`${config.BACKURL}${url}`)
    .set('x-access-token', auth.getToken())
    .set('Accept', 'application/json')
    .then(res => res.body, api.err);
api.addStoryTag = (sid, tid) => Request.post(`${config.BACKURL}/stories/${sid}/tags/${tid}`)
    .set('x-access-token', auth.getToken())
    .set('Accept', 'application/json')
    .then(res => res.body, api.err);
module.exports = api;
