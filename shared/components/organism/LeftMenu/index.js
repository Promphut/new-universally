...
class LeftMenu extends React.Component {
  ...
  shrinkDrawer = (e) => {
    ...
    let height =
      ((menu && menu.column ? menu.column : []).length + 1) * (utils.isMobile() ? 50 : 55);
  ...
  render() {
    ...
    _.remove(cols, col => _.indexOf(children, col.slug) !== -1);
      ...
      if (cols[i].slug.toLowerCase() !== 'news' || cols[i].name.toLowerCase() !== 'news') {
        items.push(
          <li key={i}>
            <a
              href="#"
              onClick={e => this.props.closeAndLink(e, `/stories/${cols[i].slug}?page=1`)}
            >
              {cols[i].name}
            </a>
          </li>,
    ...
    return (
      <Container open={open} style={{ display }}>
                ...
                <div>
                  ...
                  <li>
                    <DropDownListLink href="#" onClick={this.shrinkDrawer}>
                      Stories
                      <FontIcon className={`material-icons arrow ${toggleArrow}`}>
                        keyboard_arrow_down
                      </FontIcon>
                    </DropDownListLink>
                  </li>
