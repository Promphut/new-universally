...
export default class NextStory extends React.Component {
  ...
  handleScroll = (e) => {
    if (!utils.isMobile()) {
      ...
      const scrollOpacity = e.srcElement.body.scrollTop >
        e.srcElement.body.scrollHeight - window.innerHeight - 800;
  ...
  render() {
    if (!isEmpty(this.state.nextStory)) {
      ...
      return (
        <Link to={url || '/'}>
          <NextStoryContainer
            scrollOpacity={this.state.scrollOpacity}
            imgSrc={(cover && cover.medium) || '/pic/fbthumbnail.jpg'}
          >
            <ImgOverlay />
            <NextStoryArrow><i className="material-icons">arrow_forward</i></NextStoryArrow>
            <NextStoryHeaderWarpper>
              <NextStoryName>
                {ptitle && truncate(ptitle, { length: 40, seperator: '' })}
              </NextStoryName>
            </NextStoryHeaderWarpper>
          </NextStoryContainer>
        </Link>
